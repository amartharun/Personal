...
def extract_and_convert_to_exact_tally_format(image_path: str, output_excel_path: str):
    ...
    for index, item in enumerate(extracted_data.get("line_items", [])):
        ...
        # Calculate row-specific individual taxes dynamically based on each specific item amount
        ...
        row_entry = {
            ...
            "GSTIN": gstin_val if (gstin_val := extracted_data.get("gstin")) and is_first_row else "",
            ...
            # Document-level entries stay on row 1, but CGST and SGST are now populated for EVERY row
...
if __name__ == "__main__":
    ...
    output_excel = os.path.join(TARGET_FOLDER, "Purchase_Format_Invoice_Fixed-v2.xlsx")
