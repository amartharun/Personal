...
# =====================================================================
# 1. FIXED DATA SCHEMA WITH SUPPLIER ADDRESS
# =====================================================================
...
class TallyInvoiceSchema(BaseModel):
    ...
    hamali_charges: float = Field(default=0.0, description="Any loading, labor, or hamali charges if present (e.g., 120.00)")
    cgst_amount: float = Field(description="Total CGST tax amount calculated (e.g., 1001.98)")
    sgst_amount: float = Field(description="Total SGST tax amount calculated (e.g., 1001.98)")
...
def extract_and_convert_to_exact_tally_format(image_path: str, output_excel_path: str):
    ...
    response = client.models.generate_content(
        ...
        contents=[
            img, 
            "Extract all fields. Ensure the full supplier address is captured. Format Invoice Date strictly into YYYY-MM-DD."
        ],
    ...
    for index, item in enumerate(extracted_data.get("line_items", [])):
        ...
        # Calculate row-specific individual taxes dynamically based on the item amount
        item_amount = item.get("amount", 0.0)
        # Parse numeric rate out of string (e.g., "5%" -> 5.0)
        try:
        ...
        # Header/footer totals should ONLY appear on the first row of the invoice block
        ...
        row_entry = {
            ...
            # Document fees and round offs distributed only on row 1
            ...
            "Round Off Dr/Cr": "" if is_first_row else "",
...
if __name__ == "__main__":
    ...
    output_excel = os.path.join(TARGET_FOLDER, "Purchase_Format_Invoice_Fixed.xlsx")
